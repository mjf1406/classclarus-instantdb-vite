...
export function CreateAssignerDialog({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const transactions = [
                db.tx.equitable_assigners[assignerId].create({
                    name: name.trim(),
                    items: itemsJson,
                ...
                db.tx.equitable_assigners[assignerId].link({ class: classId }),
            ];
            ...
            <CredenzaContent className="flex flex-col max-h-[90vh]">
                <form onSubmit={handleSubmit} className="flex flex-col h-full min-h-0 overflow-hidden">
                    <CredenzaHeader className="shrink-0">
                        <CredenzaTitle>Create Equitable Assigner</CredenzaTitle>
                        <CredenzaDescription>
                            Create a new equitable assigner with items that can be
                            assigned to students with balanced experience.
                        </CredenzaDescription>
                    </CredenzaHeader>
