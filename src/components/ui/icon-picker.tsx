...
// Cache icons data to avoid reloading on every mount
let cachedIcons: IconData[] | null = null;
let iconsLoadPromise: Promise<IconData[]> | null = null;

const loadIconsData = async (): Promise<IconData[]> => {
    if (cachedIcons) {
        return cachedIcons;
    }
    ...
    iconsLoadPromise = (async () => {
        ...
        const filtered = iconsData.filter((icon: IconData) => {
            return icon.name in dynamicIconImports;
        });
        cachedIcons = filtered;
        return filtered;
    })();
...
const useIconsData = () => {
    ...
    useEffect(() => {
        ...
        loadIconsData().then((loadedIcons) => {
            if (isMounted) {
    ...
    (
        ...
        const handleOpenChange = useCallback(
            (newOpen: boolean) => {
                ...
                if (newOpen) {
                    // Measure virtualizer after popover is visible
                    setIsMeasuring(true);
        ...
        React.useEffect(() => {
            if (isPopoverVisible && !iconsLoading) {
                // Measure virtualizer when popover becomes visible and icons are loaded
                requestAnimationFrame(() => {
                    virtualizer.measure();
                });
        ...
        }, [isPopoverVisible, virtualizer, iconsLoading]);
        ...
        return (
            ...
            >
                ...
                <PopoverContent 
                    ...
                    onPointerDown={(e) => {
                        // Prevent pointer events from bubbling to dialog overlay
                        e.stopPropagation();
                    }}
                >
                    {searchable && (
                    ...
                    <div
                        ...
                        onTouchMove={(e) => {
                            // Prevent touch events from bubbling to dialog
                            e.stopPropagation();
                        }}
                        tabIndex={-1}
                    >
                        {iconsLoading || isMeasuring ? (
                            <IconsColumnSkeleton />
                        ) : (
